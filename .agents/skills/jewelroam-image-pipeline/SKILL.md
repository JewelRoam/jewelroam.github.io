...
- `article-preview.mdx`: article text with `PhotoEmbed` for independent inline images, `PhotoSequence` for consecutive inline images, or one `PhotoGallery` for gallery mode, for local review only.

For inline articles, keep the exported image order and use `PhotoSequence` only when at least two image placeholders are adjacent between text blocks. A single image remains `PhotoEmbed`; gallery mode remains `PhotoGallery`. These are explicit MDX structures, not runtime grouping rules, and they do not duplicate the underlying Photo records.
...
Do not treat `article-preview.mdx` as publishable while its description, singular `placeId`, or photo metadata is incomplete. Keep staging files under `content/inbox/`; do not place them under `content/journals/`, `content/places/`, or `content/photos/` yet.

## Preflight

1. Confirm every staged file exists, is an allowed image type, and is within the selected size limit.
2. Confirm every manifest image appears exactly once and in source order across `PhotoEmbed`/`PhotoSequence`, or in the single `PhotoGallery` for gallery mode.
...
After confirmation, create the formal `content/photos/*.json`, update the MDX under `content/journals/`, ensure the Journal's one `placeId` exists in `content/places/`, and confirm each Photo's independent `placeId` against the actual frame. A Photo place may differ from the Journal primary place when the narrative spans nearby destinations. Upload the approved release files to the configured R2 bucket, verify the public custom-domain URLs, and rerun validation/build. Report the exact objects and URLs changed.
