...
4. Read EXIF and image dimensions when available. Treat EXIF dates and GPS as proposals until confirmed. Never infer a precise location from a filename alone. Each Journal has one primary `placeId`; each Photo still gets one independently confirmed `placeId`, which may differ when an article records movement across nearby places.
