...
4. Read EXIF and image dimensions when available. Treat EXIF dates and GPS as proposals until confirmed. Never infer a precise location from a filename alone. A Journal may contain multiple Places; each Photo still gets one confirmed placeId from the Journal's placeIds.
